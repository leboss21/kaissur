...
export const createTransaction = async (req, res) => {
    try {
        ...
        const userId = req.userId || 'test-user-id'; // placeholder if auth isn't fully implemented
        if (!entrepriseId)
            return res.status(401).json({ error: 'Unauthorized' });
        const { clientId, fromCurrencyCode, toCurrencyCode, amountIn, exchangeRate, type } = req.body;

...
export const generateDailyReport = async (req, res) => {
    try {
        ...
        ops.filter(o => o.type === 'MOBILE_MONEY').forEach(o => {
            ...
            p.total += o.amount;
            p.count++;
            if (o.subType === 'DEPOSIT')
                p.deposits += o.amount;
            if (o.subType === 'WITHDRAWAL')
                p.withdrawals += o.amount;
...
export const getMonthlyReport = async (req, res) => {
    try {
        ...
        txns.forEach(t => {
            ...
            if (!byDay[day])
                byDay[day] = { exchanges: 0, mobileMoney: 0, credit: 0, tickets: 0, fees: 0 };
        ...
        ops.forEach(o => {
            ...
            if (!byDay[day])
                byDay[day] = { exchanges: 0, mobileMoney: 0, credit: 0, tickets: 0, fees: 0 };
            if (o.type === 'MOBILE_MONEY')
                byDay[day].mobileMoney += o.amount;
            if (o.type === 'CREDIT')
                byDay[day].credit += o.amount;
            if (o.type === 'TICKET')
                byDay[day].tickets += o.amount;
